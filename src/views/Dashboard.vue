<!-- views/supervisor/Dashboard.vue -->
<script setup>
...
// -------------------------
const locations = ref([])
const inventory = ref([]) // SIEMPRE lo normalizamos a array de filas
...
// Normaliza /inventory/all a filas:
// - Si viene array, lo deja.
// - Si viene objeto { "locName": [ {material, quantity}, ... ] }, lo aplana
//   y usa locations para resolver location_id y location_type.
const normalizeInventoryAll = raw => {
  // Caso ideal: backend ya devuelve lista de filas
  if (Array.isArray(raw)) return raw

  // Caso alterno: backend devuelve dict por ubicación
...
// -------------------------
// Cargar KPIs (MP_GENERAL / PT / MERMA)
// -------------------------
...
// Total de cantidad en la tabla filtrada
...
// -------------------------
// KPIs de inventario (desde endpoint)
// -------------------------
...
// -------------------------
// Merma por máquina (rows ya ordenadas)
// -------------------------
...
// Top 3 máquinas con mayor merma
    ...
    <section class="card">
      <header class="card-header">
        <div>
          ...
          <p class="subtitle">
            Vista consolidada para el supervisor, con filtros rápidos.
          </p>
        </div>
      ...
      <div class="card-body">
        ...
        <p v-else-if="errorInventory" class="error">
          {{ errorInventory }}
        </p>
        <p
      ...
      <div>
        ...
        <div class="kpi-grid">
          <div class="kpi-card">
            ...
            <p class="kpi-value">
              {{ totalsByType.MP_GENERAL.toFixed(2) }}
            </p>
          </div>
          ...
          <div class="kpi-card">
            ...
            <p class="kpi-value">
              {{ totalsByType.PT.toFixed(2) }}
            </p>
          </div>

          <div class="kpi-card">
            <p class="kpi-label">Total MERMA</p>
            <p class="kpi-value kpi-danger">
              {{ totalsByType.MERMA.toFixed(2) }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="block">
        <header class="block-header">
          <div>
            ...
            <p class="subtitle">
              Basado en los procesos registrados en MachineProcess.
            </p>
          </div>
          <button class="btn-outline" @click="loadWasteReport">
            Actualizar
          </button>
        </header>

        <p v-if="loadingWaste">Cargando merma…</p>
        <p v-else-if="errorWaste" class="error">
          {{ errorWaste }}
        </p>
        ...
        <ul v-else class="waste-list">
          ...
          >
            <div class="waste-main">
              ...
              <span class="waste-qty">
                {{ row.waste.toFixed(2) }}
              </span>
            </div>
            <p class="waste-subtitle">
              ID ubicación: {{ row.machineId }}
            </p>
          </li>
        </ul>
      </div>
